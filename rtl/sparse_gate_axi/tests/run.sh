...
record={'schema':'sparse_gate_wrapper_validation_v1','status':'NOT_RUN','source_sha256':{f:hashlib.sha256(Path(f).read_bytes()).hexdigest() for f in files},'verilator':subprocess.check_output(['verilator','--version'],text=True).strip(),'compiler':subprocess.check_output(['g++','--version'],text=True).splitlines()[0],'seed':918273,'command':'bash rtl/sparse_gate_axi/tests/run.sh'}
