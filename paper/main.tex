...
\title{Vortex-Initiated FP4 Sparse Indexing\newline on an AXI--UCIe Memory System}
\author{\IEEEauthorblockN{Anonymous Artifact Authors}\IEEEauthorblockA{Digital hardware design and reproducibility study}}
\begin{document}
\maketitle
\begin{abstract}
We implement a post-projection CSA2 indexer in synthesizable RTL on a Vortex-to-memory AXI256/AXI2Flit/UCIe path. Vortex command-processor DMA submits gate commands; gate RTL accesses the same online memory simulator through its own DMA. A 16-lane E2M1 FP4 datapath with E8M0 scales computes signed weighted-ReLU scores across 32 heads. A shared key buffer, stable Top-512 heap, and explicit FP32 rounding support full scan, candidate-only scan, and committed-index reuse. Completion follows actual memory responses. \input{generated/abstract_results.tex} Validation includes \CoreCaseCount{} core cases, \FpCaseCount{} floating-point cases, learned-weight operator fixtures, and AXI fault/backpressure checks. Projection, candidate-block generation, final attention, full-model quality, and SRAM-CIM physical implementation remain outside the demonstrated scope.
\end{abstract}
\begin{IEEEkeywords}
...
\caption{Validated Vortex CP DMA command path. Gate MMIO crosses AXI/UCIe to a C++ verification dispatcher and synthesizable SparseGate RTL. RTL DMA and ordinary accesses share online mem\_sim; responses use the same link. CPU and CoralNPU ordinary traffic is omitted from this diagram.}
\label{fig:system}
\end{figure*}
...
For Vortex-initiated commands, host software configures Vortex's command processor, but the processor's DMA master is the bus requestor for input uploads, gate register writes and status reads, and result readback. Its device address \code{0x900f0000} maps through the Vortex BAR to physical \code{0x1900f0000}. The RTL model in this configuration has a matching high-address 1-MiB data window. The CPU-initiated programs reported below are retained as separate earlier integration cases; they do not establish the Vortex command source.
...
\caption{Accepted START lifecycle. Candidate blocks are expanded externally. REUSE bypasses scoring; DONE waits for all write responses. Failures invalidate the cache, while an illegal doorbell is rejected before START.}
\label{fig:flow}
\end{figure*}
...
\input{generated/vortex_results.tex}
