...
Created with the built-in `image_gen` tool, seven calls in total: four original figures, two corrections of the system return path, and one Vortex-source revision. Exact prompts are preserved in `prompts.json`. No CLI fallback or image-editing script was used. The selected images are copied into this repository; no paper path depends on the tool's default output directory.

| File | Purpose | Review |
|---|---|---|
| system.png | Vortex CP DMA is the sole illustrated gate-command sender; SparseGate remains after UCIe and shares one online memory authority | CPU and CoralNPU ordinary sources are omitted from this command-path view. The main AXI/UCIe chain is bidirectional. No measured performance is encoded. |
