...
destination=${1:?Usage: run_vortex_gate.sh NEW_RESULT_DIRECTORY}
...
"$AXI_CXX" -O2 -Wall -Wextra -Werror -std=c++17 \
    ...
    "$AXI_PROJECT_DIR/workloads/sparse_gate_vortex_cp.cpp" \
...
"$AXI_PYTHON" "$AXI_PROJECT_DIR/scripts/check_vortex_gate.py" "$case_dir" \
    --binary "$binary" --model-manifest "$model/build.json" \
